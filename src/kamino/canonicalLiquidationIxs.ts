...
export async function buildKaminoRefreshAndLiquidateIxsCanonical(
  ...
  // 4. CORE instructions (refresh obligation)
  instructions.push(...liquidationResult.coreIxs);
  labels.push('refreshObligation');

  // 5. PRE-FARM instructions (0-2, immediately before liquidation)
 ...
 * 1. Instructions before liquidation must end with farms refreshes (0-2, required when hasFarmsRefresh=true), preceded by refreshObligation and contiguous reserve refreshes
...
export function validateCompiledInstructionWindow(
  ...
  if (cursor < 0 || kinds[cursor].kind !== 'refreshObligation') {
    let diagnostics = 'Missing refreshObligation before pre farms / liquidation\n\n';
  ...
  const preStartIdx = reserveStartIdx;
