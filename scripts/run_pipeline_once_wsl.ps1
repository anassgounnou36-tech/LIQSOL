# PowerShell script to run pipeline:once in WSL2
# This script runs the full pipeline: snapshot → candidates → scheduler → audit

# Force Ubuntu distro for all WSL calls
$Distro = "Ubuntu"

Write-Host "Running full pipeline in WSL" -ForegroundColor Cyan
Write-Host "Pipeline: snapshot:obligations → snapshot:scored → snapshot:candidates → test:scheduler:forecast → audit:pipeline" -ForegroundColor Gray
Write-Host ""
...
if ($LASTEXITCODE -ne 0) {
    ...
    Write-Host "The pipeline requires environment variables in .env file." -ForegroundColor Yellow
    exit 1
}
...
Write-Host ""
& wsl.exe -d $Distro -- bash -c "cd '$wslPath' && npm run pipeline:once"
